...
class CommentIQ_Subscriber_CommentIQAPISubscriber implements CommentIQ_EventManagement_SubscriberInterface
{
    ...
    public function on_comment_edit($comment_id)
    {
        ...
        if ($post instanceof WP_Post) {
            $this->update_cleancoded_comment($post);
        }
    }
    ...
    /**
     * Compares a potential cleancoded comment to another WordPress comment.
     *
     * To be considered, a comment must be shorter than 100 characters. If the given
     * comment doesn't match this criteria. The cleancoded comment is returned. Otherwise, the
     ...
     *
     * @param mixed      $cleancoded_comment
     * @param WP_Comment $comment
     *
     * @return mixed
     */
    private function compare_comments($cleancoded_comment, WP_Comment $comment)
    {
        if (!$this->is_valid_comment($comment)) {
            return $cleancoded_comment;
        }
        ...
        if (!$cleancoded_comment instanceof WP_Comment) {
            return $comment;
        }

        $cleancoded_comment_details = get_comment_meta($cleancoded_comment->comment_ID, $this->comment_details_meta_key, true);

        if ($comment_details['ArticleRelevance'] > $cleancoded_comment_details['ArticleRelevance']) {
            $cleancoded_comment = $comment;
        }
    ...
    /**
     * Update the cleancoded comment for the given post.
     *
     * @param WP_Post $post
     */
    private function update_cleancoded_comment(WP_Post $post)
    {
        ...
        $cleancoded_comment = array_reduce(get_comments(array('post_id' => $post->ID)), array($this, 'compare_comments'));
        ...
        if ( $post->post_author === $cleancoded_comment->user_id && ! $allow_post_author ) {
	        return;
        }

        if ($cleancoded_comment instanceof WP_Comment) {
            update_post_meta($post->ID, $this->cleancoded_comment_id_meta_key, $cleancoded_comment->comment_ID);
        }
    }
}
